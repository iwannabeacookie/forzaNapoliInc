<template>
  <div class="catalogue">
    ...
    <Item v-for="(item, index) in filteredItems" :key="index" :item="item" v-else />
  </div>
</template>
...
const applyFilter = () => {
  ...
  if (selectedSale.value) {
    filtered = filtered.filter(item => (item.sale && selectedSale.value === 'true') || (!item.sale && selectedSale.value === 'false'));
  }
